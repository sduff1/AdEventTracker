...
if(isset($_POST['Remove'])) {
    ...
    if ($checkRows=0){
        ...
        //If there is no row, then it will process the query
        $evstr = strval($event);
        $query = "SELECT ID FROM produce.promotionitem WHERE itemNumber='$item_num' AND PromoCode='$promo'";
        $result = mysqli_query($conn, $query);
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $id = $row['ID'];
            $idsub = $row['ID'];
            if(substr($id, 0, 3) == $evstr)
            {
                $query2 = "DELETE FROM produce.promotionitem WHERE itemNumber='$item_num' AND PromoCode='$promo' AND ID ='$id'";
