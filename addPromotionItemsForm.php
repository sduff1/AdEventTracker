...
//Get item number entered by user
    ...
    $promocode = mysqli_real_escape_string($conn, $_POST['promo']);
    $query = "SELECT * FROM promotion WHERE PromoCode = '$promocode'";
    if (isset($_POST['event'])) {
        $event = mysqli_real_escape_string($conn, $_POST['event']);
    }
    else{
        $event=$_SESSION['event_code'];
    }
//Get result
        ...
        <form method="POST" action="addItemtoPromotion.php">
            ...
            <table class="table" >
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    $evstr = strval($event);
                    $idquery =  mysqli_query($conn, "SELECT ID FROM produce.promotionitem WHERE ItemNumber='$itemnum'");
                    $idres = mysqli_fetch_assoc($idquery);
                    if (isset($idres['ID'])) {
                        $id = substr($idres['ID'], 0, 3);
                    }
                    else{
                        $id = 1;
                    }
                    if($id != $evstr && (mysqli_query($conn, "SELECT ItemNumber FROM produce.promotionitem WHERE ItemNumber = '$itemnum' AND PromoCode = '$code' AND ID='$id'")->num_rows == 0)){
                        echo "<tr>";
        ...
        <br>
            ...
            if($set==true) {
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    if ($id == $evstr && (mysqli_query($conn, "SELECT ItemNumber FROM produce.promotionitem WHERE ItemNumber = '$itemnum' AND PromoCode = '$code'")->num_rows != 0)) {
                        echo '<tr>';
                        echo '<td><input type = "radio" name = "itemNumber" value="' . $itemnum . '" class="form-control" />&nbsp;</td>>';
                        for ($i = 0; $i < count($column_arr); $i++) {
                            echo '<td>' . $row[$column_arr[$i]] . '</td>';
